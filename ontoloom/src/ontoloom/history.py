...
def _revert_del(ont: Ontology, event: EventRecord) -> tuple[bool, str]:
    ...
    if json_data is None or json_data[0] is None:
        return False, f"skip revert-del [{h[:HASH_DISPLAY_LEN]}]: no stored axiom_json"
...
def _revert_replace(ont: Ontology, event: EventRecord) -> tuple[bool, str]:
    ...
    if old_h is None:
        return False, f"skip revert-replace [{new_h[:HASH_DISPLAY_LEN]}]: no replaces_hash"
    ...
    if old_json_row is None or old_json_row[0] is None:
        return (
            False,
            f"skip revert-replace [{new_h[:HASH_DISPLAY_LEN]}->{old_h[:HASH_DISPLAY_LEN]}]: no stored json for old axiom",
        )
...
def _revert_annotate(ont: Ontology, event: EventRecord) -> tuple[bool, str]:
    ...
    axiom = load_axiom(json_data, f"revert-annotate {h[:HASH_DISPLAY_LEN]}")
