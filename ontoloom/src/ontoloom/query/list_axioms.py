...
class ListAxioms(FrozenModel):
    constraints: tuple[AxiomConstraint, ...]
    limit: int | None = None
    offset: int = 0

    @model_validator(mode="after")
    def _validate_pagination(self) -> "ListAxioms":
        if self.offset < 0:
...
def normalize(q: ListAxioms) -> ListAxioms:
    return q.model_copy(update={"constraints": normalize_axiom(q.constraints)})


def render(q: ListAxioms) -> CompiledSql:
...
def _run(s: Session, q: ListAxioms) -> list[tuple[AxiomHash, str]]:
    compiled = render(normalize(q))
    return [(AxiomHash(r[0]), r[1]) for r in s._conn.execute(compiled.sql, compiled.params)]
