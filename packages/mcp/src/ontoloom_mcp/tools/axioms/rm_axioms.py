...
def rm_axioms(
    ...
    - `select`: Format "name@hash_prefix" (e.g. "my_sel@a3f1"). The hash prefix
    ...
    """
    if select is not None and hash_prefixes is not None:
        msg = "Cannot use both 'select' and 'hash_prefixes'. Choose one."
        raise ValueError(msg)
    if select is None and hash_prefixes is None:
